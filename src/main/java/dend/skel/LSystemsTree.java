...
public class LSystemsTree implements PlugIn {
	...
	public void run(final String ignored) {
		...
		final ImagePlus imp= IJ.altKeyDown() ?
			lst.createTreeStack("Fractal Tree") :
			lst.sampleTree();
		imp.show();
	}
	...
	public ImagePlus createTreeStack(final String title) {
		...
		final ImagePlus imp= IJ.createImage(title, "8-bit black", width, height,
			10);
	...
	public ImagePlus createTree(final String title, final int width,
		final int height, final double angle,
	...
	private void drawTree(final ImageProcessor ip, final int x1, final int y1,
		final double angle,
		final int recursions) {
		...
		if (recursions < 0)
			return;
		final int x2= x1 +
			(int) (Math.cos(Math.toRadians(angle)) * recursions * 10);
		final int y2= y1 +
			(int) (Math.sin(Math.toRadians(angle)) * recursions * 10);
