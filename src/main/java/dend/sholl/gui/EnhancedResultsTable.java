...
public class EnhancedResultsTable extends ResultsTable {
	...
	/** Calls the default {@code incrementCounter()} while monitoring unsaved measurements */
	@Override
	...
	/** Calls the default {@code show()} method while attaching a WindowListener used to prompt users to
	 * save unsaved measurements when closing (non-programmatically) the ResultsTable window
	 *
	 * @param windowTitle the title of the window displaying the ResultsTable */
	@Override
	public void show(final String windowTitle) {
		...
		window.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(final WindowEvent e) {
				try {
					final TextWindow tw= (TextWindow) e.getWindow();
					final EnhancedResultsTable ert= (EnhancedResultsTable) tw
						.getTextPanel().getResultsTable();
					if (!(ert.getUnsavedMeasurements() && ert.getCounter() > 0))
						return;
					final ImageJ ij= IJ.getInstance();
					final boolean macro= IJ.macroRunning() ||
					...
					IJ.log(
						">>>> An error occurred when closing table:\n" + exc);
				}
			}
	...
	/** Updates the ResultsTable and displays it in its own window if this ResultsTable is already being
	 * displayed, otherwise displays the contents of this ResultsTable in new window with the specified
	 * title.
	 *
	 * @param fallBackTitle The window title of the new TextWindow to be opened if current table is not
	 *                      being displayed */
	...
	private synchronized boolean promptForSave(final String tableTitle) {
		...
		gd.addMessage("Save measurements in " + tableTitle + "?",
			new Font("SansSerif", Font.BOLD, 12));
		gd.addMessage("Data will be discarded if you dismiss this prompt!",
